...
截至 **2026-06-21**（三期验收冻结）：

- 一期 / 二期主工作流已完整跑通（Sprint 1～10 已关闭）
- 三期正式立项，Sprint 11～18 已按本主控交付并关闭
...
| Migration | 内容 | Sprint |
|-----------|------|--------|
| 014 | `allow_asset_reuse` on project | 15 |
| 013 | 素材 `vision_analysis` JSON + status | 14 |
| 015 | OAuth token 表（provider、user、refresh、expires） | 16 |
...
5. OpenAI OAuth 手工授权成功（Codex 订阅登录已走通）；Google OAuth / Gemini 订阅 UI 已隐藏且无配额，不作为三期关闭阻塞项（决策 4）
6. 部署仍为单机 SQLite，无 Redis 依赖（决策 5）
7. E2E + pytest 全绿；`phase3-backlog.md` 剩余项标四期

**状态：三期已关闭（立项 2026-06-26；验收冻结 2026-06-21）**
