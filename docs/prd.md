...
- 项目概览展示历史方案的主题、镜头数、时长、镜头顺序和标题差异，并允许恢复指定版本。恢复前自动备份当前方案，仅回退主题、分镜和导出文案；素材、真实音频和节拍校准保持当前状态，历史版本引用缺失素材时禁止恢复。
- 项目概览支持主题、完整分镜和导出文案单步骤重跑。每次重跑保存前后版本，只覆盖用户选择的阶段；其中分镜重跑必须满足真实音频节拍就绪条件。
- 分镜列表支持选择 1 至 8 个连续镜头进行局部重跑，并允许填写自然语言调整要求。局部重跑只重新选择素材和改写镜头文案，固定镜头 ID、叙事角色、起止时间、节拍点及区间外分镜；执行前后均保存版本快照，便于模型效果对比和恢复。
- JSON 型 LLM 生成任务失败后，前端在当前页面保存最近一次接口和请求参数，并提供“原参数重试”。重试成功后刷新当前业务数据；用户主动取消不进入重试状态。音频上传不持久化本地文件内容，失败后需重新选择文件。
- Codex / OAuth 授权失效时，即使当前步骤已使用规则结果兜底，也保留原页面、接口和请求参数，并明确引导用户重新连接；连接恢复后可返回原页面按原参数重跑，不要求重新填写业务输入。
