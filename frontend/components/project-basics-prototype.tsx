...
const styleOptions = [
  {
    value: "情绪氛围片",
    label: "情绪氛围片",
    description: "适合雪景、山野、清晨、空镜较多的素材，重氛围和沉浸感。"
  },
  {
    value: "路线纪实片",
    label: "路线纪实片",
    ...
    startTransition(async () => {
      try {
        const payload: ProjectPayload = {
          ...form,
          stylePreference: customStyle.trim()
            ? `${form.stylePreference}；补充：${customStyle.trim()}`
            : form.stylePreference,
          status: "draft"
        };
        ...
        setError(
          submitError instanceof Error ? submitError.message : "保存项目失败，请稍后重试。"
        );
      }
    });
  }
